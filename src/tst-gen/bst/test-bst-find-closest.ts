...
type TreeType = { root: string, nodes: BSTNodeType[] };
type CaseType = { target: number, tree: TreeType, output: number };

const bstFindClosetCases: CaseType[] =
    [{
        "target": 12,
        "tree": {
            "nodes": [
...
export const genTest = (): void => {
    bstFindClosetCases.forEach((item, index) => {
        console.log(`test('Test Case #${index + 1}', function () {
        const root = buildTreeWithId( treeCases.tree2.nodes, treeCases.tree2.root);
        expect(findClosestValueInBst(root!, ${item.target})).toBe(${item.output});
    });`)
    });
}